...
public class PdfService : IPdfService
{
    ...
    public byte[] GenerateServisCalismaRaporuPdf(List<ServisCalisma> calismalar, DateTime baslangic, DateTime bitis)
    {
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Element(c => ComposeHeader(c, $"SERVÝS ÇALIÞMA RAPORU\n{baslangic:dd.MM.yyyy} - {bitis:dd.MM.yyyy}"));
    ...
    public byte[] GenerateBelgeUyariRaporuPdf(List<BelgeUyari> uyarilar)
    {
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Element(c => ComposeHeader(c, "BELGE SÜRE UYARI RAPORU"));
    ...
    public byte[] GenerateCariEkstresPdf(Cari cari, List<Fatura> faturalar, List<BankaKasaHareket> hareketler)
    {
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Element(c => ComposeHeader(c, $"CARÝ EKSTRE\n{cari.Unvan}"));
    ...
    private void ComposeHeader(IContainer container, string title)
    {
        container.Row(row =>
        {
            ...
            {
                col.Item().Text("CRM FÝLO SERVÝS").Bold().FontSize(16);
            ...
            if (fatura.FaturaKalemleri?.Any() == true)
            {
                col.Item().Table(table =>
                {
                    ...
                    table.Header(header =>
                    {
                        ...
                        header.Cell().Background(Colors.Grey.Lighten2).Padding(5).Text("Açýklama").Bold();
    ...
    private void ComposeServisCalismaContent(IContainer container, List<ServisCalisma> calismalar)
    {
        container.Column(col =>
        {
            ...
            col.Item().Row(row =>
            {
                ...
                {
                    c.Item().Text("Araç Sayýsý").FontSize(9);
            ...
            col.Item().Table(table =>
            {
                table.ColumnsDefinition(columns =>
                {
                    ...
                    columns.RelativeColumn(1);   // Þoför
                ...
                table.Header(header =>
                {
                    ...
                    header.Cell().Background(Colors.Grey.Lighten2).Padding(5).Text("Güzergah").Bold();
                    header.Cell().Background(Colors.Grey.Lighten2).Padding(5).Text("Firma").Bold();
                    header.Cell().Background(Colors.Grey.Lighten2).Padding(5).Text("Þoför").Bold();
        ...
        container.Column(col =>
        {
            ...
            col.Item().Table(table =>
            {
                table.ColumnsDefinition(columns =>
                {
                    ...
                    columns.RelativeColumn(2);   // Belge Türü
                    columns.RelativeColumn(1);   // Bitiþ Tarihi
                    columns.RelativeColumn(1);   // Kalan Gün
                ...
                table.Header(header =>
                {
                    ...
                    header.Cell().Background(Colors.Grey.Lighten2).Padding(5).Text("Belge Türü").Bold();
                    header.Cell().Background(Colors.Grey.Lighten2).Padding(5).Text("Bitiþ Tarihi").Bold();
                    header.Cell().Background(Colors.Grey.Lighten2).Padding(5).AlignCenter().Text("Kalan Gün").Bold();
                ...
                foreach (var u in uyarilar.OrderBy(x => x.KalanGun))
                {
                    ...
                    table.Cell().Background(bgColor).BorderBottom(1).BorderColor(Colors.Grey.Lighten2).Padding(3).AlignCenter().Text(u.KalanGun < 0 ? $"{Math.Abs(u.KalanGun)} gün geçti" : $"{u.KalanGun} gün");
                    table.Cell().Background(bgColor).BorderBottom(1).BorderColor(Colors.Grey.Lighten2).Padding(3).Text(u.Seviye switch
                    {
                        BelgeUyariSeviye.Kritik => "KRÝTÝK",
                        BelgeUyariSeviye.Acil => "ACÝL",
        ...
        container.Column(col =>
        {
            ...
            col.Item().Border(1).Padding(10).Row(row =>
            {
                row.RelativeItem().Column(c =>
                {
                    ...
                    c.Item().PaddingTop(5).Text("Ünvan:").Bold();
            ...
            if (faturalar.Any())
            {
                ...
                col.Item().Table(table =>
                {
                    ...
                    table.Header(header =>
                    {
                        ...
                        header.Cell().Background(Colors.Grey.Lighten2).Padding(5).Text("Tarih").Bold();
                        header.Cell().Background(Colors.Grey.Lighten2).Padding(5).AlignRight().Text("Tutar").Bold();
                        header.Cell().Background(Colors.Grey.Lighten2).Padding(5).AlignRight().Text("Ödenen").Bold();
