...
public class MuhasebeService : IMuhasebeService
{
    ...
    public async Task<NakitAkisRapor> GetNakitAkisRaporuAsync(int yil, int? ay = null)
    {
        ...
        #region Muhasebeleştirme Yönetimi

        /// <summary>
        /// Muhasebeleştirme özet bilgilerini getirir
        /// </summary>
        public async Task<MuhasebeleştirmeOzet> GetMuhasebeleştirmeOzetAsync(int yil, int ay)
        {
            var ayBaslangic = new DateTime(yil, ay, 1, 0, 0, 0, DateTimeKind.Utc);
            var ayBitis = ayBaslangic.AddMonths(1).AddDays(-1);

            // Muhasebeleşmemiş faturalar
            var muhasebeleşmemişFaturalar = await _context.Faturalar
                .AsNoTracking()
                .Where(f => !f.IsDeleted && !f.MuhasebeFisiOlusturuldu)
                .ToListAsync();

            // Muhasebeleşmemiş masraflar
            var muhasebeleşmemişMasraflar = await _context.AracMasraflari
                .AsNoTracking()
                .Where(m => !m.IsDeleted && m.MuhasebeFisId == null)
                .ToListAsync();

            // Aylık muhasebeleşen fişler
            var aylikFisler = await _context.MuhasebeFisleri
                .AsNoTracking()
                .Where(f => !f.IsDeleted && f.FisTarihi >= ayBaslangic && f.FisTarihi <= ayBitis)
                .ToListAsync();

            return new MuhasebeleştirmeOzet
            {
                MuhasebeleşmemişFaturaSayisi = muhasebeleşmemişFaturalar.Count,
                MuhasebeleşmemişFaturaToplam = muhasebeleşmemişFaturalar.Sum(f => f.GenelToplam),
                MuhasebeleşmemişMasrafSayisi = muhasebeleşmemişMasraflar.Count,
                MuhasebeleşmemişMasrafToplam = muhasebeleşmemişMasraflar.Sum(m => m.Tutar),
                GelenFaturaSayisi = muhasebeleşmemişFaturalar.Count(f => f.FaturaYonu == FaturaYonu.Gelen),
                GelenFaturaToplam = muhasebeleşmemişFaturalar.Where(f => f.FaturaYonu == FaturaYonu.Gelen).Sum(f => f.GenelToplam),
                GidenFaturaSayisi = muhasebeleşmemişFaturalar.Count(f => f.FaturaYonu == FaturaYonu.Giden),
                GidenFaturaToplam = muhasebeleşmemişFaturalar.Where(f => f.FaturaYonu == FaturaYonu.Giden).Sum(f => f.GenelToplam),
                AylikMuhasebeleşenFis = aylikFisler.Count,
                AylikMuhasebeleşenToplam = aylikFisler.Sum(f => f.ToplamBorc)
            };
        }

        /// <summary>
        /// Muhasebeleştirilmemiş faturaları getirir
        /// </summary>
        public async Task<List<MuhasebeleştirmeFaturaItem>> GetMuhasebeleşmemişFaturalarAsync(DateTime? baslangic = null, DateTime? bitis = null)
        {
            ...
            var faturalar = await query.OrderByDescending(f => f.FaturaTarihi).ToListAsync();

            return faturalar.Select(f => new MuhasebeleştirmeFaturaItem
            {
                Id = f.Id,
                FaturaNo = f.FaturaNo,
                FaturaTarihi = f.FaturaTarihi,
                CariUnvan = f.Cari?.Unvan ?? "",
                FaturaYonu = f.FaturaYonu == FaturaYonu.Gelen ? "Gelen" : "Giden",
        ...
        /// <summary>
        /// Muhasebeleştirilmemiş masrafları getirir
        /// </summary>
        public async Task<List<MuhasebeleştirmeMasrafItem>> GetMuhasebeleşmemişMasraflarAsync(DateTime? baslangic = null, DateTime? bitis = null)
        {
            ...
            var masraflar = await query.OrderByDescending(m => m.MasrafTarihi).ToListAsync();

            return masraflar.Select(m => new MuhasebeleştirmeMasrafItem
            {
                Id = m.Id,
                MasrafTarihi = m.MasrafTarihi,
                AracPlaka = m.Arac?.Plaka ?? "",
                MasrafKalemi = m.MasrafKalemi?.Adi ?? "",
                BelgeNo = m.BelgeNo,
                Tutar = m.Tutar,
                Aciklama = m.Aciklama,
                CariUnvan = m.Cari?.Unvan,
                Secili = false
            }).ToList();
        }

        /// <summary>
        /// Masraf için muhasebe fişi oluşturur
        /// Masraf: 770 Giderler + 191 İndirilecek KDV BORÇ, 320 Satıcılar veya 100/102 Kasa/Banka ALACAK
        /// </summary>
        public async Task<MuhasebeFis> CreateMasrafFisiAsync(AracMasraf masraf)
        {
            // Masrafı tam yükle
            var masrafEntity = await _context.AracMasraflari
                .Include(m => m.Arac)
                .Include(m => m.MasrafKalemi)
                .Include(m => m.Cari)
                .FirstOrDefaultAsync(m => m.Id == masraf.Id);

            if (masrafEntity == null)
                throw new Exception("Masraf kaydı bulunamadı");
            ...
            {
                FisNo = fisNo,
                FisTarihi = DateTime.SpecifyKind(masrafEntity.MasrafTarihi, DateTimeKind.Utc),
                FisTipi = FisTipi.Mahsup,
                Aciklama = $"Araç Masrafı: {masrafEntity.Arac?.Plaka ?? ""} - {masrafEntity.MasrafKalemi?.Adi ?? ""}" +
                           (!string.IsNullOrEmpty(masrafEntity.BelgeNo) ? $" (Belge: {masrafEntity.BelgeNo})" : ""),
                Kaynak = FisKaynak.Diger,
                KaynakId = masrafEntity.Id,
            ...
            // KDV oranı - masraf kalemi üzerinde olabilir veya varsayılan
            var kdvOrani = masrafEntity.MasrafKalemi?.KdvOrani ?? 20;
            var kdvTutar = masrafEntity.Tutar * kdvOrani / (100 + kdvOrani);
            var netTutar = masrafEntity.Tutar - kdvTutar;

            // Gider hesabı belirleme - masraf kalemine göre
            var giderHesapKodu = masrafEntity.MasrafKalemi?.VarsayilanMuhasebeKodu ?? "770.01";
            var giderHesap = await GetHesapByKodAsync(giderHesapKodu) ?? await GetHesapByKodAsync("770");
            ...
            if (giderHesap != null)
            {
                kalemler.Add(new MuhasebeFisKalem
                {
                    ...
                    Borc = netTutar,
                    Alacak = 0,
                    Aciklama = $"Gider: {masrafEntity.MasrafKalemi?.Adi ?? ""}",
                    SiraNo = siraNo++
                });
            }
            ...
            // Karşı hesap - Cari varsa Satıcılar (320), yoksa Kasa (100)
            if (masrafEntity.CariId.HasValue)
            {
                var saticilarHesap = await GetOrCreateCariHesapAsync("320", masrafEntity.CariId.Value);
                kalemler.Add(new MuhasebeFisKalem
                {
                    HesapId = saticilarHesap.Id,
                    Borc = 0,
                    Alacak = masrafEntity.Tutar,
                    Aciklama = $"Satıcı: {masrafEntity.Cari?.Unvan ?? ""}",
                    CariId = masrafEntity.CariId,
                    SiraNo = siraNo++
                });
            }
            else
            {
                // Kasa çıkışı varsay
                var kasaHesap = await GetHesapByKodAsync("100.01") ?? await GetHesapByKodAsync("100");
                if (kasaHesap != null)
                {
                    kalemler.Add(new MuhasebeFisKalem
                    {
                        HesapId = kasaHesap.Id,
                        Borc = 0,
                        Alacak = masrafEntity.Tutar,
                        Aciklama = "Kasa Çıkışı",
                        SiraNo = siraNo++
                    });
                }
            }
            ...
            // Masrafa fiş ID'sini kaydet
            masrafEntity.MuhasebeFisId = fis.Id;
            await _context.SaveChangesAsync();
        ...
        /// </summary>
        public async Task<TopluMuhasebeleştirmeSonuc> TopluFaturaMuhasebeleştirAsync(List<int> faturaIdler)
        {
            var sonuc = new TopluMuhasebeleştirmeSonuc();
            ...
            foreach (var faturaId in faturaIdler)
            {
                try
                {
                    ...
                    if (fatura == null || fatura.MuhasebeFisiOlusturuldu)
                        continue;

                    var fis = await CreateFaturaFisiAsync(fatura);
                    sonuc.OlusturulanFisIdler.Add(fis.Id);
                    sonuc.BasariliFisSayisi++;
                    sonuc.IslenmisFaturaSayisi++;
                }
                catch (Exception ex)
                {
                    sonuc.HataliFisSayisi++;
                    sonuc.Hatalar.Add($"Fatura ID {faturaId}: {ex.Message}");
                }
            }

            sonuc.Basarili = sonuc.HataliFisSayisi == 0;
            return sonuc;
        }

        /// <summary>
        /// Toplu masraf muhasebeleştirme
        /// </summary>
        public async Task<TopluMuhasebeleştirmeSonuc> TopluMasrafMuhasebeleştirAsync(List<int> masrafIdler)
        {
            var sonuc = new TopluMuhasebeleştirmeSonuc();

            foreach (var masrafId in masrafIdler)
            {
                try
                {
                    var masraf = await _context.AracMasraflari
                        ...
                        .Include(m => m.MasrafKalemi)
                        .Include(m => m.Cari)
                        .FirstOrDefaultAsync(m => m.Id == masrafId);

                    if (masraf == null || masraf.MuhasebeFisId != null)
                        continue;

                    var fis = await CreateMasrafFisiAsync(masraf);
                    sonuc.OlusturulanFisIdler.Add(fis.Id);
                    sonuc.BasariliFisSayisi++;
                    sonuc.IslenmisMasrafSayisi++;
                }
                catch (Exception ex)
                {
                    sonuc.HataliFisSayisi++;
                    sonuc.Hatalar.Add($"Masraf ID {masrafId}: {ex.Message}");
                }
            }
        ...
        /// <summary>
        /// Muhasebeleştirme kontrol listesi Excel export
        /// </summary>
        public async Task<byte[]> ExportMuhasebeleştirmeKontrolListesiAsync(DateTime baslangic, DateTime bitis)
        {
            var kontrolListesi = new List<MuhasebeKontrolSatiri>();

            // Faturalar
            var faturalar = await _context.Faturalar
                .AsNoTracking()
                .Include(f => f.Cari)
                .Include(f => f.MuhasebeFisi)
                .Where(f => !f.IsDeleted && f.FaturaTarihi >= baslangic && f.FaturaTarihi <= bitis)
                .OrderBy(f => f.FaturaTarihi)
                .ToListAsync();

            foreach (var f in faturalar)
            {
                kontrolListesi.Add(new MuhasebeKontrolSatiri
                {
                    KaynakTip = f.FaturaYonu == FaturaYonu.Gelen ? "Gelen Fatura" : "Giden Fatura",
                    KaynakId = f.Id,
                    BelgeNo = f.FaturaNo,
                    Tarih = f.FaturaTarihi,
                    Aciklama = f.Cari?.Unvan ?? "",
                    Tutar = f.GenelToplam,
                    MuhasebeFisiVar = f.MuhasebeFisiOlusturuldu,
                    MuhasebeFisId = f.MuhasebeFisId,
                    FisNo = f.MuhasebeFisi?.FisNo,
                    Durum = f.MuhasebeFisiOlusturuldu ? "Muhasebeleştirildi" : "Bekliyor"
                });
            }
            ...
            foreach (var m in masraflar)
            {
                kontrolListesi.Add(new MuhasebeKontrolSatiri
                {
                    KaynakTip = "Araç Masrafı",
                    KaynakId = m.Id,
                    BelgeNo = m.BelgeNo ?? "-",
                    Tarih = m.MasrafTarihi,
                    Aciklama = $"{m.Arac?.Plaka ?? ""} - {m.MasrafKalemi?.Adi ?? ""}",
                    Tutar = m.Tutar,
                    MuhasebeFisiVar = m.MuhasebeFisId.HasValue,
                    MuhasebeFisId = m.MuhasebeFisId,
                    FisNo = m.MuhasebeFis?.FisNo,
                    Durum = m.MuhasebeFisId.HasValue ? "Muhasebeleştirildi" : "Bekliyor"
                });
            }

            // Excel oluştur
            using var workbook = new ClosedXML.Excel.XLWorkbook();
            var ws = workbook.Worksheets.Add("Kontrol Listesi");

            // Başlık
            ws.Cell(1, 1).Value = "MUHASEBELEŞTİRME KONTROL LİSTESİ";
            ws.Cell(1, 1).Style.Font.Bold = true;
            ws.Cell(1, 1).Style.Font.FontSize = 14;
            ...
            // İstatistikler
            var bekleyenFatura = kontrolListesi.Count(x => x.KaynakTip.Contains("Fatura") && !x.MuhasebeFisiVar);
            var bekleyenMasraf = kontrolListesi.Count(x => x.KaynakTip == "Araç Masrafı" && !x.MuhasebeFisiVar);
            ws.Cell(3, 1).Value = $"Bekleyen Fatura: {bekleyenFatura} | Bekleyen Masraf: {bekleyenMasraf}";
            ws.Range(3, 1, 3, 9).Merge();

            // Tablo başlıkları
            var headers = new[] { "Kaynak Tipi", "ID", "Belge No", "Tarih", "Açıklama", "Tutar", "Durum", "Fiş No" };
            for (int i = 0; i < headers.Length; i++)
            {
                ws.Cell(5, i + 1).Value = headers[i];
                ws.Cell(5, i + 1).Style.Font.Bold = true;
                ws.Cell(5, i + 1).Style.Fill.BackgroundColor = ClosedXML.Excel.XLColor.LightBlue;
                ws.Cell(5, i + 1).Style.Border.OutsideBorder = ClosedXML.Excel.XLBorderStyleValues.Thin;
            }

            // Veri satırları
            int row = 6;
            foreach (var item in kontrolListesi.OrderBy(x => x.Tarih))
            {
                ws.Cell(row, 1).Value = item.KaynakTip;
                ws.Cell(row, 2).Value = item.KaynakId;
                ws.Cell(row, 3).Value = item.BelgeNo;
                ws.Cell(row, 4).Value = item.Tarih.ToString("dd.MM.yyyy");
                ws.Cell(row, 5).Value = item.Aciklama;
                ws.Cell(row, 6).Value = item.Tutar;
                ws.Cell(row, 6).Style.NumberFormat.Format = "#,##0.00 ₺";
                ws.Cell(row, 7).Value = item.Durum;
                ws.Cell(row, 8).Value = item.FisNo ?? "-";

                // Bekleyen kayıtları sarı ile işaretle
                if (!item.MuhasebeFisiVar)
                {
                    ws.Range(row, 1, row, 8).Style.Fill.BackgroundColor = ClosedXML.Excel.XLColor.LightYellow;
                }
            ...
            ws.Cell(row, 6).Value = kontrolListesi.Sum(x => x.Tutar);
