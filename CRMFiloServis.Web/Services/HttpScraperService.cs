...
public class HttpScraperService : IHttpScraperService
{
    ...
    private decimal ParseFiyat(string text)
    {
        ...
        // "1.850.000 TL" veya "1,850,000" formatlarý
    ...
    private int ParseKilometre(string text)
    {
        ...
        // "125.000 km" veya "125,000 km" formatlarý
    ...
    private string? ParseKoltukSayisi(string text)
    {
        ...
        // "5 koltuk", "7 kiþilik", "5+2", "5 kisilik" gibi formatlar
        var patterns = new[]
        {
            @"(\d)\s*(?:\+\s*\d)?\s*koltuk",           // 5 koltuk, 5+2 koltuk
            @"(\d)\s*(?:\+\s*\d)?\s*kiþilik",          // 5 kiþilik
            @"(\d)\s*(?:\+\s*\d)?\s*kisilik",          // 5 kisilik
            @"(\d)\s*\+\s*(\d)\s*koltuk",              // 5+2 koltuk -> 7
            @"(\d)\s*kapý",                            // 5 kapý
            @"(\d)\s*kapi",                            // 5 kapi
            @"\b([2-9])\s*(?:kiþi|kisi)\b",           // 5 kiþi
            @"(?:sedan|hatchback|station).+?(\d)\s*k", // sedan 5 kapý
        };
        ...
        foreach (var pattern in patterns)
        {
            ...
            if (match.Success)
            {
                // 5+2 formatý için toplam hesapla
    ...
    private DateTime ParseTarih(string text)
    {
        ...
        var gunMatch = Regex.Match(lowerText, @"(\d+)\s*(?:gün|gun)\s*(?:önce|once)");
